...
REMOTE="$(curl -sSf --max-time 5 \
    https://raw.githubusercontent.com/seangalabin/laravel-code-reviewer/master/skill/VERSION \
...
if [[ "$LOCAL" != "$REMOTE" ]]; then
    echo ""
    echo "⚠️  code-reviewer is out of date (installed: ${LOCAL:-unknown}, latest: $REMOTE)."
    echo "   Update before continuing:"
    echo ""
    echo "     npx github:seangalabin/laravel-code-reviewer"
    echo ""
    exit 1
fi
