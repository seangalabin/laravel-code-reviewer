...
Optional knobs: `AI_REVIEW_MAX_USD` (spend cap, default 2.00 — a runaway ceiling, you pay actual usage; don't set below ~1.00 or a mid-run kill bills tokens but posts nothing), `AI_REVIEW_MODEL` (e.g. `sonnet` — the biggest cost lever, 3–5× cheaper than opus), `AI_REVIEW_OUTPUT` (JSON output path). Exit codes: `0` ran, `1` infra failure (missing env / no `claude` CLI), `2` the run errored. Works on any host with the `claude` CLI on `PATH`.
