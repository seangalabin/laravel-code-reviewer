...
Read every change for what happens when it **fails**, not only for whether the happy path works — "it works" is not sufficient. Most dimensions below already carry that lens: partial state (§8), external calls (§10), production data volumes (§9, §16), concurrency (§16e).
...
- A new Model for **reference/lookup data the deployment doesn't own** when the project's central data source already carries it — see §11 *Centralized reference data* (canonical); the model belongs on the shared connection via the established pattern, not on a local table.
...
- **Slow or external work inside a `DB::transaction()` closure** — an HTTP call, a file / S3 upload, a shell-out, or long-running processing — 🟡 Warning. The transaction holds its database connection and every row lock it has already taken for as long as the third party takes to answer, so another service's latency spike arrives here as lock contention and connection-pool exhaustion. Keep the closure to the writes: make the call before it, or split into *write a pending row → call → commit the result in a second short transaction* when the outcome must be persisted. **Don't flag** a closure whose only slow work is more SQL, or a queue / `Cache::` call already deferred with `afterCommit` — the bullet above owns those.
- **An external resource created before the database row that references it** — an S3 / disk upload, a payment-provider customer or charge, a third-party record — followed by the `create()` / `update()` that stores its identifier — 🟡 Warning. `DB::transaction()` cannot roll back the other system: if that write fails (validation, deadlock, a crash between the two), the remote object survives with nothing pointing at it — invisible to the app, never cleaned up, and often still billed. Persist first and treat the external call as the retryable step, or write the returned identifier in its own immediate write so a reconciliation job can find the orphans. **Don't flag** a genuinely temporary resource with failure-safe cleanup (§16g), or a call whose result the code deliberately discards.
...
- **Sorting, filtering, or aggregating in PHP what the database can do in SQL** — `->get()->sortBy(...)`, `->get()->filter(...)`, `->get()->sum()` / `->max()` / `->groupBy()` on a Collection, or `->all()` then `array_filter` — 🟡 Warning on the same unbounded / growing tables as the full-table-loads rule above. Every row is hydrated into a model first, so memory and time scale with the table while the database would answer from an index. Push the work onto the builder: `->orderBy()`, `->where()`, `->sum('amount')` / `->selectRaw('sum(amount) …')`, `groupBy` in SQL. **Don't flag** the same shape on a small bounded reference set, on a collection already fully loaded for another reason (the rows are needed anyway — sorting them in PHP saves a query), or a sort/filter on a **computed or appended** value SQL has no column for.
- **Unbounded page size** — `->paginate($request->per_page)` / `->paginate($request->input('limit'))` / `->simplePaginate($size)` where the value comes from request input with no ceiling — 🟡 Warning. Pagination the caller can set to `100000` is a full-table load one query-string parameter away, reachable by anyone who can hit the endpoint. Clamp it (`min($request->integer('per_page', 25), 100)`) or bound it in the FormRequest (`'per_page' => 'integer|max:100'`). **Don't flag** a page size that is a constant, a config value, or already validated with a `max:` rule.
...
- **Retry policy and rate limiting on an external call** — 🔵 Suggestion, and a judgement call rather than a default. `Http::` already needs a timeout (§9); retry is the separate question of what to do when that timeout fires. `->retry(3, 200)` is cheap insurance on an **idempotent** read (a `GET`, a lookup) where a transient blip is the likely failure. It is **not** safe on a non-idempotent write — a retried `POST` that creates an order or takes a payment can do it twice unless the endpoint accepts an **idempotency key**; recommend the key first and the retry second, never the retry alone. Retrying a `4xx` other than `429` only burns the timeout budget on a request that will never succeed. Treat **`429` as its own case**: it is the server asking for a pause, so honour `Retry-After` or back off exponentially instead of retrying immediately or logging it as a generic failure — a retry storm is how a throttle becomes a block. **Do not** ask for `retry()` on every call; raise it where the integration is visibly flaky, the operation is safely repeatable, or a documented rate limit exists.
...
- **Schema cohesion — a nameable column cluster added to an existing entity table** — 🔵 Suggestion. When a migration adds columns to an existing table and the resulting group passes all three tests, suggest a separate table (a typed 1:1 detail table, or a proper child table when rows repeat) instead of widening the entity. **The column count is a signal, not the gate — and judge the whole cluster, not just this diff's additions:** two new columns extending an existing same-family group already on the table fire (the cluster is what's grown); two novel unrelated columns usually don't — a two-column detail table over-normalizes, buying a join, a model, and mapping overhead to relocate two fields. The task's context decides: if the card is about a separable concern (features, preferences, marketing data), separation applies even at small counts.
  ...
  3. **Growth-shaped** — a same-prefix or flag-family pattern that is accreting: count existing family columns on the table plus this migration's, and read the card for whether more of the same are coming.
...
- **Column home-check — does a related table already own this concept?** — 🔵 Suggestion. Before a migration adds a column to an existing entity table, check whether a **related table already owns the concept** the column describes. Look it up cheaply: read the **entity model's relationship methods first** (one file, authoritative for what detail/child tables exist — `x_features`, `x_settings`, `x_details`, `x_metadata`); only consult `database/migrations/` when the model isn't conclusive. If the new column belongs to that related concept rather than to the entity itself, it goes on the related table. Signals: the column name shares the related table's noun; sibling columns of the same family already live there. Don't flag when: no such related table exists (the cluster rule above decides instead); the column is read on the entity's hot paths together with entity columns (the join trade-off again); the related table is a pivot whose grain doesn't match; or the column is a **denormalized aggregate of the related data** — counter caches and rollups (`x_count`, `x_total`, `last_x_at`) share the related noun *by design* and intentionally live on the parent to avoid the join. Phrase to confirm: "`x_features` already owns this concept — should `{column}` live there instead of on `{entity}`?"
- **Centralized reference data — consume the central source, don't re-home it locally.** 🟡 Warning when the diff creates or extends a **reference / lookup dataset** in this deployment's own database — a `*_lookup` / reference-table migration, a Model for it, a seeder or factory of geo / catalogue / registry / price data, or a hardcoded list of such values — for data the deployment does **not own** (regions, suburbs, countries, product catalogues, price lists, a client registry) **and** the project's **central data source already carries it**. Find the central source the way the project declares it (`CLAUDE.md`: a shared DB connection, an internal API) and probe it through the **established consumer pattern** — models on the shared connection under their own namespace, the internal API client — plus the central schema when it's reachable. Per-deployment copies drift from the source of truth and ship one migration per deployment. Name the pattern in the finding ("consume this via the existing `<Namespace>\X` model on the shared connection instead of a local table"). **Unclear whether the dataset is central** → 🔵 confirm-not-accuse, and say so: *if in doubt, verify with the data owner named in the project `CLAUDE.md` before duplicating.* Fires equally on **extending an existing local copy** (new columns/rows in a table that mirrors a central one) — same fix direction. **Don't flag:** data genuinely owned per deployment (this client's own records); a **deliberate local cache/sync** of central data — only when the source of truth and the refresh path are visible (a sync job/command in the diff or already in the repo), otherwise it's a copy; factories/fixtures used only by tests; a brand-new dataset no central source carries yet (its *shape* is judged by schema cohesion above).
...
- **Async submit with no in-flight guard** — a `@click` / `@submit` / `onSubmit` handler that calls an async action without checking a `submitting` / `loading` flag and disabling the control — 🟡 Warning. A double-click, or an impatient re-click during a slow response, sends the request twice; against a non-idempotent endpoint that is two orders, two charges, or two rows. Guard the handler (`if (this.submitting) return`) **and** bind `:disabled` / `disabled` to the same flag — the visual disable alone still loses the race on a fast double-click. (§8 owns the server-side half; a client guard is not a substitute for one.)
- **A request per keystroke** — a search / filter / autocomplete input whose `@input` / `onChange` / watcher calls the API directly with no debounce — 🟡 Warning. A ten-character query issues ten requests, nine already obsolete, and the load multiplies by every user typing at once. Debounce the handler (~300ms), using the project's existing utility if it has one, or trigger on submit / blur instead. **Don't flag** an input filtering an already-loaded client-side list, or one already wrapped in a debounce helper.
